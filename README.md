...
# With vision metrics tracking (enabled by default)
python -m holdem.cli.run_dry_run \
  --profile assets/table_profiles/default_profile.json \
  --policy runs/blueprint/avg_policy.json \
...
# Disable vision metrics if needed
python -m holdem.cli.run_dry_run \
  --profile assets/table_profiles/default_profile.json \
  --policy runs/blueprint/avg_policy.json \
...
**💡 NEW: Vision Metrics Tracking** is now enabled by default in dry-run and auto-play modes! The system continuously monitors:
- OCR accuracy and latency
- Amount recognition errors (MAE/MAPE)
- Card recognition accuracy per street
- Performance metrics (parse latency)
- Real-time alerts for threshold violations

See [VISION_METRICS_INTEGRATION.md](VISION_METRICS_INTEGRATION.md) for complete documentation.
...
- **[VisionMetrics Integration](VISION_METRICS_INTEGRATION.md)** - **NEW**: Real-time vision monitoring enabled by default in all CLI commands
